    ...
    const items = [
        {
            template: () => (
                <span className="inline-flex align-items-center gap-1 px-2 py-2">
                    ...
                    <span onClick={handleMinimize}> <Avatar image="https://api.dicebear.com/9.x/adventurer/svg?seed=Gizmo" className="mr-2" shape="circle" /></span>
                </span>
            )
        },
        {
            ...
            template: (item, options) => (
                <button onClick={(e) => options.onClick(e)} className={classNames(options.className, 'w-full p-link flex align-items-center p-2 pl-4 text-color hover:surface-200 border-noround ')}>
                    ...
                    <div className="flex flex-column align">
                        <span className="font-bold ">Oussema Amri</span>
            ...
            items: [
                {
                    ...
                    template: (item, options) => (
                        <div className='p-menuitem-content'>
                            <a className="flex align-items-center p-menuitem-link" onClick={handleProfile}>
                                <span className={item.icon} />
                                <span className="mx-2">{item.label}</span>
                                {item.badge && <Badge className="ml-auto" value={item.badge} />}
                                {item.shortcut && <span className="ml-auto border-1 surface-border border-round surface-100 text-xs p-1">{item.shortcut}</span>}
                ...
                },
                {
                    ...
                    template: (item, options) => (
                        <div className='p-menuitem-content'>
                            <a className="flex align-items-center p-menuitem-link" onClick={handleRegister}>
                                <span className={item.icon} />
                                <span className="mx-2">{item.label}</span>
                                {item.badge && <Badge className="ml-auto" value={item.badge} />}
                                {item.shortcut && <span className="ml-auto border-1 surface-border border-round surface-100 text-xs p-1">{item.shortcut}</span>}
    ...
    return (
        <div style={{ position: 'absolute', top: '10px', right: '10px' }}>
            ...
            {extend ? (
                ...
                <div className="card flex justify-content-center" onClick={handleExtend}>
                    <Avatar image="https://api.dicebear.com/9.x/adventurer/svg?seed=Gizmo" className="mr-2" shape="circle" />
                </div>
            )}
        </div>
    );
