import { Controller, Post, Get, Param, Body, Delete, Put, Query } from '@nestjs/common';
...
export class ProjectController {
    ...
    getProjectsByProjectManager(@Param('projectManagerId') projectManagerId: number) {
        return this.projectService.getProjectsByProjectManager(projectManagerId);
    }

    @Put(':id')
    updateProject(@Param('id') id: number, @Body() updateProjectDto: CreateProjectDto) {
        return this.projectService.updateProject(id, updateProjectDto);
    }

    @Post(':projectId/users/:userId')
    assignUserToProject(@Param('projectId') projectId: number, @Param('userId') userId: number) {
        return this.projectService.assignUserToProject(projectId, userId);
    }

    @Delete(':projectId/users/:userId')
    unassignUserFromProject(@Param('projectId') projectId: number, @Param('userId') userId: number) {
