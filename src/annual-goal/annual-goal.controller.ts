...
export class AnnualGoalController {
    ...
    @Get('approval')
    getGoalsByApprovalStatus(@Query('approvedBy') approvedBy: 'employee' | 'manager', @Query('approved') approved: boolean) {
        return this.annualGoalService.getGoalsByApprovalStatus(approvedBy, approved);
    }

    @Put(':id')
    updateGoal(@Param('id') id: number, @Body() updateGoalDto: UpdateAnnualGoalDto) {
        return this.annualGoalService.updateGoal(id, updateGoalDto);
    }

    @Put(':id/approve')
    approveGoal(@Param('id') id: number, @Body() approvalDto: { approvedBy: 'employee' | 'manager' }) {
